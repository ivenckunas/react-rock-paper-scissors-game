...
function App() {
  ...
  const [getColor, setColor] = useState("#fff")
  ...
  function addNum(e) {
    ...
    if (num === "Add 1") setNum(getNum + 1)
    if (num === "Add 3") setNum(getNum + 3)
    if (num === "Add 5") setNum(getNum + 5)

  }

  const [getBg, setBg] = useState('#fff')

  function changeBg() {
    if (getNum >= 10 && getNum <= 20) setBg("green");
    if (getNum >= 20) setBg("blue");
  }
  ...
  return (
    <div className='container'>
      ...
      <div className="game-container">
        <h1>CHOOSE ONE:</h1>
        {choices.map((choice, i) => <button key={i} onClick={() => handleClick(choice)} > {choice} </button>)}
      ...
      <div className="num-container">
        <div className='box' style={{ backgroundColor: getBg }}>Num: {getNum}</div>

        <button onClick={addNum}>Add 1</button>
        <button onClick={addNum}>Add 3</button>
      ...
      <div className="box-container">
        {colors.map((el, i) => <ColorMap color={el} key={i} />)}
      </div>
